...
// "\x27""###;
    ...
    // let n: usize = inputs.map(|s| string_code_length(s) - _string_memory_length(s)).sum();
    let n: usize = inputs.map(|s| string_length_encoded(s) - string_code_length(s) ).sum();

    println!("difference: {}", n);
}

fn string_code_length(input: &str) -> usize {
    input.len()
}

fn _string_memory_length(input: &str) -> usize {
    if input.len() < 3 {
        return 0;
    }
    let mut len= 0usize;
...
fn string_length_encoded(input: &str) -> usize {
    input.chars()
        .map(|c| match c {
            '\\' | '"' => 2,
            _ => 1
