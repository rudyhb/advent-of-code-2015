...
pub(crate) fn run() {
    let _input = "children: 3, cats: 7, samoyeds: 2, pomeranians: 3, akitas: 0, vizslas: 0, goldfish: 5, trees: 3, cars: 2, perfumes: 1";
    let sample: CompoundValues = _input.parse().unwrap();

    let _input = _get_input();
    let sues = _input.split('\n').map(|line| {
        let line = &line[line.chars().position(|c| c == ':').unwrap() + 2..];
        line.parse().unwrap()
    }).collect::<Vec<CompoundValues>>();

    let sue = sues.iter().enumerate().filter(|(_, sue)| sample.is_superset_of(sue))
        .map(|(i, sue)| (i + 1, sue))
        .collect::<Vec<_>>();
    match sue.len() {
        1 => {
            println!("matching is Sue {}: {:?}", sue[0].0, sue[0].1);
        }
        num => {
            println!("there are {} matching Sues", num);
        }
    }

    let sue = sues.iter().enumerate().filter(|(_, sue)| sample.is_superset_v2_of(sue))
        .map(|(i, sue)| (i + 1, sue))
        .collect::<Vec<_>>();
    match sue.len() {
        1 => {
            println!("v2 matching is Sue {}: {:?}", sue[0].0, sue[0].1);
        }
        num => {
            println!("there are {} matching Sues", num);
        }
...
struct CompoundValues(HashMap<CompoundType, u32>);

impl Debug for CompoundValues {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0.iter().map(|(val, num)| format!("{:?}: {}", val, num)).collect::<Vec<String>>().join(", "))
    }
}

impl CompoundValues {
    pub(crate) fn is_superset_of(&self, subset: &Self) -> bool {
        self.0.iter().all(|(key, val)| {
            match subset.0.get(key) {
                None => true,
                Some(value_subset) => value_subset == val
            }
        })
    }
    pub(crate) fn is_superset_v2_of(&self, subset: &Self) -> bool {
        self.0.iter().all(|(key, val)| {
            match subset.0.get(key) {
                None => true,
                Some(value_subset) => {
                    match key {
                        CompoundType::Cats => return val < value_subset,
                        CompoundType::Dogs(dog) => {
                            match dog {
                                DogType::Pomeranians => return val > value_subset,
                                _ => {}
                            }
                        }
                        CompoundType::Goldfish => return val > value_subset,
                        CompoundType::Trees => return val < value_subset,
                        _ => {}
                    }
                    value_subset == val
                }
...
impl FromStr for CompoundValues {
    type Err = ();

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let map: Result<HashMap<CompoundType, u32>, Self::Err> =
            s.split(", ").map(|value| {
                let mut parts = value.split(": ");
                let name = match parts.next().ok_or(())? {
                    "children" => CompoundType::Children,
                    "cats" => CompoundType::Cats,
                    "goldfish" => CompoundType::Goldfish,
                    "trees" => CompoundType::Trees,
                    "cars" => CompoundType::Cars,
                    "perfumes" => CompoundType::Perfumes,
                    "samoyeds" => CompoundType::Dogs(DogType::Samoyeds),
                    "pomeranians" => CompoundType::Dogs(DogType::Pomeranians),
                    "akitas" => CompoundType::Dogs(DogType::Akitas),
                    "vizslas" => CompoundType::Dogs(DogType::Vizslas),
                    _ => return Err(())
                };
                let value: u32 = parts.next().ok_or(())?.parse().or(Err(()))?;
                Ok((name, value))
            }).collect();
