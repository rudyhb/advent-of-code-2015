...
pub fn run() {
    ...
    let mut grid = Grid::of_size(1_000);
    let inputs = input.split('\n');
    for command in inputs {
        let (action, subgrid) = parse_command(command);
        match action {
            TurnOn => grid.turn_on(subgrid),
            TurnOff => grid.turn_off(subgrid),
            Toggle => grid.toggle(subgrid)
        }
    }

    println!("brightness: {}", grid.number_on());
}

fn parse_command(command: &str) -> (Action, SubGrid) {
    lazy_static! {
        static ref REG: Regex = Regex::new(r"^(?P<action>(?:turn on)|(?:turn off)|(?:toggle)) (?P<x1>\d+),(?P<y1>\d+) through (?P<x2>\d+),(?P<y2>\d+)$").unwrap();
    }

    let caps = REG.captures(command).unwrap();

    let action: Action = match &caps["action"] {
        "turn on" => TurnOn,
        "turn off" => TurnOff,
        "toggle" => Toggle,
        _ => panic!("out of range action")
    };

    let parse = |a: &str| caps[a].parse::<usize>().unwrap();

    let grid = SubGrid {
        top_left: Point { x: parse("x1"), y: parse("y1") },
        bottom_right: Point { x: parse("x2"), y: parse("y2") }
    };
...
impl Grid {
    ...
    pub fn of_sizes(x: usize, y: usize) -> Self {
        Self(vec![vec![false; x]; y])
    }
    pub fn number_on(&self) -> usize {
        self.0.iter().map(|row| row.iter().filter(|&cell| *cell).count()).sum()
    }
    fn turn_on(&mut self, subgrid: SubGrid) {
        for x in subgrid.top_left.x..=subgrid.bottom_right.x {
            for y in subgrid.top_left.y..=subgrid.bottom_right.y {
                self.0[y][x] = true;
            }
        }
    }
    fn turn_off(&mut self, subgrid: SubGrid) {
        for x in subgrid.top_left.x..=subgrid.bottom_right.x {
            for y in subgrid.top_left.y..=subgrid.bottom_right.y {
                self.0[y][x] = false;
            }
    ...
    fn toggle(&mut self, subgrid: SubGrid) {
        for x in subgrid.top_left.x..=subgrid.bottom_right.x {
            for y in subgrid.top_left.y..=subgrid.bottom_right.y {
...
impl Grid2 {
    ...
    pub fn of_sizes(x: usize, y: usize) -> Self {
        Self(vec![vec![0u32; x]; y])
    }
    pub fn number_on(&self) -> u32 {
        self.0.iter().map(|row| row.iter().sum::<u32>()).sum()
    }
    fn turn_on(&mut self, subgrid: SubGrid) {
        for x in subgrid.top_left.x..=subgrid.bottom_right.x {
            for y in subgrid.top_left.y..=subgrid.bottom_right.y {
