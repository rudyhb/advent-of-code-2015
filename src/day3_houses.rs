...
pub fn how_many_houses(input: &str) -> usize {
    let mut hashmap: HashSet<Point> = HashSet::new();
    let mut location = Point::origin();
    hashmap.insert(location);
...
pub fn how_many_houses_robot_santa(input: &str) -> usize {
    let mut hashmap: HashSet<Point> = HashSet::new();
    let mut location = Point::origin();
    let mut location_robot = Point::origin();
    hashmap.insert(location);
    ...
    for (i, direction) in input.chars().enumerate() {
        if i % 2 == 0 {
            ...
            location_robot += map(direction);
            hashmap.insert(location_robot);
        }
    }
...
pub fn get_houses_input() -> &'static str {
...
pub fn run() {
    let input = get_houses_input();
    // let how_many = how_many_houses(input);
    let how_many = how_many_houses_robot_santa(input);
    println!("Santa goes to {} houses", how_many);
}